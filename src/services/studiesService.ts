...
class StudiesService {
  async getStudies(page: number, size: number) {
    try {
      ...
      const studies = await pool.query(
        "SELECT * FROM studies LIMIT $1 OFFSET $2",
        [size, offset]
      );
  ...
  async getStudyById(id: number) {
    try {
      const study = await pool.query("SELECT * FROM studies WHERE id = $1", [
        id,
      ]);
      return study.rows[0];
    } catch (error) {
  ...
  async getStudiesByPatientDni(dni: string) {
    try {
      const studies = await pool.query(
        "SELECT * FROM studies WHERE patient_id = (SELECT id FROM patients WHERE dni = $1)",
        [dni]
      );
      return studies.rows;
    } catch (error) {
  ...
  async createStudy(
    ...
    try {
      const newStudy = await pool.query(
        "INSERT INTO studies (patient_id, type, status, date) VALUES ($1, $2, $3, $4) RETURNING *",
        [patient_id, type, status, date]
      );
  ...
  async updateStudy(
    ...
    try {
      const updatedStudy = await pool.query(
        "UPDATE studies SET patient_id = $1, type = $2, status = $3, date = $4 WHERE id = $5 RETURNING *",
        [patient_id, type, status, date, id]
      );
  ...
  async uploadStudy(id: number, result_file_path: string) {
    try {
      const updatedStudy = await pool.query(
        "UPDATE studies SET result_file_path = $1 WHERE id = $2 RETURNING *",
        [result_file_path, id]
      );
      return updatedStudy.rows[0];
    } catch (error) {
  ...
  async deleteStudy(id: number) {
    try {
      const deletedStudy = await pool.query(
        "DELETE FROM studies WHERE id = $1 RETURNING *",
        [id]
      );
