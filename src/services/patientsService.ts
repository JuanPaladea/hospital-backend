...
class PatientsService {
  ...
  async getPatientByDni(dni: string) {
    try {
      const patient = await pool.query("SELECT * FROM patients WHERE dni = $1", [dni]);
      return patient.rows[0];
    } catch (error) {
      throw error
    }
  ...
  async getPatientDetails(id: number) {
    try {
      const patient = await pool.query("SELECT * FROM patients WHERE id = $1", [id]);
      const studies = await pool.query("SELECT * FROM studies WHERE patient_id = $1", [id]);
      const bills = await pool.query("SELECT * FROM bills WHERE patient_id = $1", [id]);
  ...
  async createPatient(name: string, age: string, dni: string) {
    try {
      const newPatient = await pool.query("INSERT INTO patients (name, age, dni) VALUES ($1, $2, $3) RETURNING *", [name, age, dni]);
  ...
  async updatePatient(id: number, name: string, age: string, dni: string) {
    try {
      const updatedPatient = await pool.query("UPDATE patients SET name = $1, age = $2, dni = $3 WHERE id = $4 RETURNING *", [name, age, dni, id]);
